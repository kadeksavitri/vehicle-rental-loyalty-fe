...
import { canCreateBooking, getCurrentUser, isCustomer } from '@/lib/rbac'
...
// Filter bookings for customer - only show their own bookings
const filteredBookings = computed(() => {
  ...
  return bookings.value.filter((booking) => booking.customerId === user.id)
})
...
<template>
  <div class="min-h-screen bg-gray-50 p-6 font-sans">
    ...
    <div class="max-w-6xl mx-auto bg-white shadow-lg ring-1 ring-gray-200/70 rounded-2xl p-6">
      ...
      <VDataTable :data="filteredBookings" :columns="columns" :page-size="10" class="mt-4" />
    </div>
  </div>
</template>
