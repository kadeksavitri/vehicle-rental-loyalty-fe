...
import type {
  ...
  DeleteRentalBookingRequest,
  RentalBooking,
  UpdateRentalBookingAddOnRequest,
  UpdateRentalBookingStatusRequest,
...
export const useRentalBookingStore = defineStore('rentalbooking', {
  ...
  actions: {
    async fetchRentalBookings(rentalVendorId?: string, status?: string, vehicleId?: string) {
      ...
      try {
        const response = await apiClient.get<CommonResponseInterface<RentalBooking[]>>('/bookings')
    ...
    async getRentalBooking(id: string) {
      ...
      try {
        const response = await apiClient.get<CommonResponseInterface<RentalBooking>>(
          `/bookings/${id}`,
        )
    ...
    async createRentalBooking(rentalBookingData: CreateRentalBookingRequest) {
      ...
      try {
        const response = await apiClient.post<CommonResponseInterface<RentalBooking>>(
          '/bookings/create',
          rentalBookingData,
        )
    ...
    async updateRentalBooking(rentalBookingData: RentalBooking) {
      ...
      this.error = null
      try {
        const response = await apiClient.put<CommonResponseInterface<RentalBooking>>(
          '/bookings/update-details',
          rentalBookingData,
        )
        if (response.status === 200 || response.status === 201) {
        ...
        const serverMessage =
          err?.response?.data?.message || (err instanceof Error ? err.message : 'Unknown error')
    ...
    async deleteRentalBooking(deleteData: DeleteRentalBookingRequest): Promise<boolean> {
      ...
      try {
        const response = await apiClient.delete<CommonResponseInterface<RentalBooking>>(
          `/bookings/${deleteData.id}/delete`,
          { data: deleteData },
        )
        if (response.status === 200 || response.status === 201) {
    ...
    async chartRentalBookings(data: { period: string; year: number }) {
      ...
      try {
        const res = await apiClient.get<
          CommonResponseInterface<{ label: string; count: number }[]>
        >('/bookings/chart', { params: data })
