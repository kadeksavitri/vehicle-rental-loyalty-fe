...
export const useVehicleStore = defineStore('vehicle', {
  ...
  actions: {
    async fetchVehicles() {
      ...
      try {
        const response = await apiClient.get<CommonResponseInterface<Vehicle[]>>('/vehicles')
    ...
    async getVehicle(id: string) {
      ...
      try {
        const response = await apiClient.get<CommonResponseInterface<Vehicle>>(`/vehicles/${id}`)
        return response.data.data
      } catch (error) {
        this.error = error instanceof Error ? error.message : 'Unknown error'
        toast.error(`Error saat memuat kendaraan: ${this.error}`)
    ...
    async createVehicle(vehicleData: CreateVehicleRequest) {
      ...
      try {
        const response = await apiClient.post<CommonResponseInterface<Vehicle>>(
          '/vehicles/create',
          vehicleData,
        )
    ...
    async updateVehicle(vehicleData: UpdateVehicleRequest) {
      ...
      try {
        const response = await apiClient.put<CommonResponseInterface<Vehicle>>(
          `/vehicles/update/${vehicleData.id}`,
          vehicleData,
        )
    ...
    async deleteVehicle(id: string) {
      ...
      this.error = null
      try {
        const response = await apiClient.delete<CommonResponseInterface<Vehicle>>(
          `/vehicles/delete/${id}`,
        )
        if (response.status === 200 || response.status === 201) {
